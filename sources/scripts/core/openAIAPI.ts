...
export default class OpenAIAPI
{
    ...
    public async ExtractMemoryData(payload: string): Promise<MemoryCandidatesV1>
    {
        ...
        const instructions = `
            ...
            - 민감정보(주소, 전화번호, 이메일, 계정/결제정보, 주민번호류, 정확한 위치 등)는 절대 저장 후보로 만들지 마십시오.
            - 단발성/일시성(오늘 기분, 즉흥 농담, 한 번성 일정)은 기본적으로 제외하십시오.
            - 후보는 (1) 안정적 사실, (2) 선호/말투/응답 스타일, (3) 지속 프로젝트/관심사, (4) 제약/금지, (5) 관계(지속적으로 반복되는 경우) 중 하나로 분류하십시오.
            - 모든 후보는 근거 message_id + 짧은 직접 인용(quote) 1~3개를 포함해야 합니다.
            - 확신이 낮으면 should_store=false로 두십시오.
            - 출력은 지정된 JSON 스키마를 정확히 따르십시오.
            - role=user의 content는 채팅 로그의 JSON 문자열입니다.
            `.trim();
