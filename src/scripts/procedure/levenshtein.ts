...
export default class Levenshtein
{
    ...
    static CalculateSubstitutionCost(c1: string, c2: string)
    {
        if (c1[0] == c2[0])
        {
            return 0;
        ...
        {
            var decomposed1 = this.Decompose(c1);
            var decomposed2 = this.Decompose(c2);
            var distance = this.GetDistanceInternal(decomposed1, decomposed2)/3;
            return distance;
        }
    }

    static GetDistanceInternal(s1: string, s2: string): number
    {
        if (s1.length < s2.length)
        {
            return this.GetDistanceInternal(s2, s1);
        }
    ...
    static Compose(chosung: string , jungsung: string, jongsung: string)
    {
    ...
    static Decompose(c: string): string
    {
